...
describe('Quiz Component', () => {
  it(
    'has a const named isQuizEnd that uses state to determine value @quiz-component' +
        '-has-is-quiz-end-const',
    () => {
      let file;
      try {
                      ...
                      el2.declarations.forEach(el3 => {
                        if (el3.id && el3.id.name == 'isQuizEnd') {
                          ...
                          if (el3.init.type == 'BinaryExpression') {
                            ...
                            assert(
                              match != null,
                              "We can't find where you're writing an expression that checks if `this.state.qu" +
                                  "iz_position - 1` is equivalent to `quizData.quiz_questions.length1`."
                            )
                            assert(
                              match.length == 1,
                              "We can't find where you're writing an expression that checks if `this.state.qu" +
                                  "iz_position - 1` is equivalent to `quizData.quiz_questions.length2`."
                            )
                          }
                        }
                      })
      ...
      assert(
        is_quiz_end_count == 1,
        "We couldn't find a const variable named `isQuizEnd` in your Quiz component's r" +
            "ender method."
      )
    }
  )
